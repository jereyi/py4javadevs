...
export function addLesson(req, res, next) {
    ...
    .then(() => res.status(200).send(`Successfully added ${req.body.lesson} to User with netid ${req.session.cas.netid}`))
        .catch((err) => {
...
export function deleteLesson(req, res, next) {
    ...
    .then(() => res.status(200).send(`Successfully removed ${req.body.lesson} from User with netid ${req.session.cas.netid}`))
        .catch((err) => {
